...
const FamiliesGallery = () => {
  ...
  const familyImages = [
    ...
    {
      id: 2,
      src: 'https://images.unsplash.com/photo-1495195134817-aeb325a55b65?w=800&h=1200&fit=crop',
      alt: 'Family moment',
      orientation: 'portrait'
    },
    {
    ...
    {
      id: 4,
      src: 'https://images.unsplash.com/photo-1495195134817-aeb325a55b65?w=1200&h=800&fit=crop',
  ...
  const prevImage = () => {
    if (selectedImage !== null) {
      setSelectedImage(selectedImage === 0 ? familyImages.length - 1 : selectedImage - 1);
    }
  };
      ...
      <div className={`fixed top-0 left-0 right-0 z-50 backdrop-blur-md border-b ${
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <h1 className={`text-xl font-semibold ${theme === 'dark' ? 'text-green-400' : 'text-green-600'}`}>Families Gallery</h1>
      ...
      <div className="pt-20 pb-8 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className={`text-center mb-12 scroll-fade-in ${galleryAnimation.isVisible ? 'visible' : ''}`} ref={galleryAnimation.ref}>
            <h2 className={`text-3xl md:text-4xl font-elegant mb-4 ${theme === 'dark' ? 'text-green-400' : 'text-green-600'}`}>Family Stories</h2>
            <p className={`text-lg ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>Precious connections and lasting memories</p>
          </div>
      ...
      {selectedImage !== null && (
        <div className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center">
          <div className="relative max-w-7xl max-h-full p-4">
            ...
            <img
              src={familyImages[selectedImage].src}
              alt={familyImages[selectedImage].alt}
              className="max-w-full max-h-[90vh] object-contain rounded-lg"
            />
            
            <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-black/50 backdrop-blur-sm text-white px-4 py-2 rounded-full text-sm">
              {selectedImage + 1} / {familyImages.length}
            </div>
          </div>
        </div>
      )}
